...
template <typename T>
VecR<int> VecR<T>::minimumValue(){
    VecR<int> index (0, this->n);
//    cout << this->n << endl;
    int min = this->v[0];
    int index0 = 0;
...
//        cout << i << endl;
//        cout << "min : " << min << " vs " << this->v[i] << endl;
        if (this->v[i] < min) {
            min = this->v[i];
//            cout << "updated min : " << min << endl;
            index0 = i;
//            cout << "updated index : " << index0;
        }
    }

    index.addValue(index0);
//    cout << "final min : " << index0;
    for(int i=0;i<this->n;i++){
        if(this->v[i] == min){
            index.addValue(i);
        }
...
template <typename T>
VecR<int> VecR<T>::rank(){
    VecR<int> R (this->n);
    for(int i=0;i<n;i++){
        int r=1,s=1;
        for(int j=0;j<n;j++){
            if (j!=i && this->v[j]<this->v[i]) r+=1;
            if (j!=i && this->v[j]<this->v[i]) s+=1;
        }
